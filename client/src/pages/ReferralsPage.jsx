...
export default function ReferralsPage() {
  ...
  async function updateTelegramUser(user, alertLevel, referralIds) {
    try {
      setSaving(true); setError(''); setNotice('')
      // A chat first appears here as pending (authorized: false). Assigning an
      // access level is the approval action, so do not carry that pending flag
      // back to the API when the admin saves the form.
      await api.put(`/referrals/telegram-users/${encodeURIComponent(user.chat_id)}`, { alertLevel, authorized: true, referralIds })
      await load(); setNotice('Telegram access saved and activated.')
    } catch (requestError) { setError(apiErrorMessage(requestError)) } finally { setSaving(false) }
  }
