      ...
      <div className="visitors-workspace">
        <article className="panel visitor-list-panel">
          ...
          <div className="table-wrap"><table className="visitors-table"><thead><tr><th>Signal</th><th>Last page</th><th>Environment</th><th>Visits</th><th>Last seen</th><th aria-label="Open details" /></tr></thead><tbody>
            {loading ? <tr className="visitor-state-row"><td colSpan="6">Loading visitor records…</td></tr> : null}
            {!loading && !filteredVisitors.length ? <tr className="visitor-state-row"><td colSpan="6">No matching signals. Open <code>/visitor</code> to create a test visit.</td></tr> : null}
            {!loading && filteredVisitors.map((visitor) => <tr className={selectedVisitor?.id === visitor.id ? 'selected' : ''} key={visitor.id}><td data-label="Signal"><strong>#{visitor.id}</strong><span className="muted-text">{display(visitor.ip_address)}</span></td><td data-label="Last page"><span className="page-cell">{display(visitor.last_page || visitor.current_page)}</span></td><td data-label="Environment">{display(visitor.device_type)}<span className="muted-text">{[visitor.browser, visitor.os].filter(Boolean).join(' · ') || '—'}</span></td><td data-label="Visits">{Number(visitor.visit_count || 0).toLocaleString()}</td><td data-label="Last seen">{formatDate(visitor.last_visit)}</td><td className="visitor-open-cell"><button className="icon-button" title={`View signal #${visitor.id}`} type="button" onClick={() => openVisitor(visitor.id)}><FiChevronRight aria-hidden="true" /></button></td></tr>)}
          </tbody></table></div>
        </article>
